...
class LogisticRegression(object):
    ...
    """
	    ...
	    p=T.cast(T.sum(y),dtype="float32")+0.1
	    n=T.cast(T.sum(T.ones_like(y))-T.sum(y),dtype="float32")+0.1
            tp=T.cast(T.sum(y)-T.sum(T.lt(self.y_pred,y)),dtype="float32")
            fp=T.cast(T.sum(T.lt(y,self.y_pred)),dtype="float32")
	    tn=T.cast(T.sum(T.ones_like(y))-T.sum(y)-T.sum(T.lt(y,self.y_pred)),dtype="float32")
            fn=T.cast(T.sum(T.lt(self.y_pred,y)),dtype="float32")
	    sn = tp/p
	    sp = tn/n
	    st = (sn+sp)/2
...
class LeNetConvPoolLayer(object):
    ...
    def predict_gram(self, new_data, batch_size):
        """
        predict for new data
        """
        img_shape = (batch_size, 1, self.image_shape[2], self.image_shape[3])
    ...
    def predict_weight(self, new_data, batch_size):
	return self.W
