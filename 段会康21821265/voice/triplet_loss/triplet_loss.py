...
def _pairwise_distances(embeddings, squared=False):
    ...
    distances = tf.expand_dims(
        square_norm, axis=1) - 2.0 * dot_product + tf.expand_dims(
            square_norm, axis=0)
...
def _get_anchor_positive_triplet_mask(labels):
    ...
    labels_equal = tf.equal(
        tf.expand_dims(labels, 0), tf.expand_dims(labels, 1))  # labels相等，
...
def batch_hard_triplet_loss(labels, embeddings, margin, squared=False):
    ...
    anchor_positive_dist = tf.multiply(mask_anchor_positive,
                                       pairwise_distances)
    hardest_positive_dist = tf.reduce_max(
        anchor_positive_dist, axis=1, keepdims=True)  # 取每一行最大的值即为最大positive距离
    ...
    max_anchor_negative_dist = tf.reduce_max(
        pairwise_distances, axis=1, keepdims=True)  # 每一样最大值
    anchor_negative_dist = pairwise_distances + max_anchor_negative_dist * (
        1.0 - mask_anchor_negative)  # (1.0 - mask_anchor_negative)即为invalid位置
