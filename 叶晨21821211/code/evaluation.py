...
def visualization(Embeddings, y, pic_name, rnd=2020):
    X_tsne = TSNE(n_components=2, init='pca', random_state=rnd, perplexity=30).fit_transform(Embeddings)
    plt.figure(figsize=(10, 5))
    plt.subplot(121)
    plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c=y)
    plt.legend()
    plt.savefig('embed/{}.png'.format(pic_name), dpi=120)
